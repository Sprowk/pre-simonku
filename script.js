...
// ============================================================
const surprises = {
    1: { title: "Pracovitá ako včielka", poem: "Od rána do večera lietaš,\nsladký med do života mi vpletáš.\nUsilovná, nezastavíš sa snáď,\npreto ťa mám tak veľmi rád.", scene: 'bee' },
    2: { title: "Divoká ako chobotnica", poem: "Účes divý ako chápadlá,\ndo tvojich sietí si ma hneď chytila.\nNa koncerte si vtedy riadne žila,\ntvoja energia ma úplne opantala.", scene: 'octopus' },
    3: { title: "Krásna kočka", poem: "Kráčaš sťa ladná micka,\nsi moja najdrahšia opička... vlastne kočka!\nOči žiaria, úsmev hreje,\npri tebe sa moje srdce len tak smeje.", scene: 'cat' },
    4: { title: "Vždy rozžiariš", poem: "Keď je vonku chladne a lialo by,\ntvoj úsmev ma vždy zahreje bez obavy.\nSi moje slnko, čo mraky odháňa,\ns tebou je každý deň plný usmievania.", scene: 'sun' },
    ...
    6: { title: "Milý tučniačik", poem: "Občas možno trošku milo zmätená,\nno pre mňa si tá najvzácnejšia odmena.\nAko tučniačik kráčaš smelo dňom,\ns tebou je každý okamih krásnym snom.", scene: 'penguin' },
    0: { title: "Magický jednorožec", poem: "Magická, vzácna, úplne iná,\ns tebou je každá chvíľa len nevinná.\nSi môj jednorožec z rozprávky,\nodháňaš preč všetky obavy.", scene: 'unicorn' }
};

// ============================================================
// SCENES (kept from original — abbreviated helpers)
// ============================================================
function makeFlowers(n){const C=[['#FF6B9D','#FF4081','#FF80AB'],['#CE93D8','#BA68C8','#E1BEE7'],['#FFD54F','#FFB300','#FFE082'],['#FF8A65','#FF5722','#FFAB91'],['#81D4FA','#29B6F6','#B3E5FC']];let h='';// spread flowers evenly but with a little jitter so they don't stack
const used=[];for(let i=0;i<n;i++){const c=C[i%C.length];let x;do{x=6+Math.random()*88;}while(used.some(u=>Math.abs(u-x)<9));used.push(x);const ht=28+Math.random()*20,d=(i*0.4).toFixed(1),dur=(2.5+Math.random()*2).toFixed(1);h+=`<g transform="translate(${x},${50-ht})" style="animation:sway ${dur}s ease-in-out ${d}s infinite;transform-origin:${x}px 50px;"><line x1="0" y1="${ht}" x2="0" y2="0" stroke="#43A047" stroke-width="2.5" stroke-linecap="round"/><circle cx="0" cy="-3" r="6" fill="${c[0]}"/><circle cx="5" cy="2" r="6" fill="${c[1]}"/><circle cx="-5" cy="2" r="6" fill="${c[2]}"/><circle cx="0" cy="2" r="4" fill="#FFC107"/></g>`}return h}
